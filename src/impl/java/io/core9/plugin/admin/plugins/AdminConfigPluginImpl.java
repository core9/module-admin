...
public class AdminConfigPluginImpl extends AbstractAdminPlugin implements
	...
	protected void process(Request request) {
		if (request.getParams().get("listtypes") != null) {
			request.getResponse().sendJsonArray(
					repository.getConfigTypes(request.getVirtualHost()));
		} else {
	...
	protected void process(Request request, String type) {
		...
		case POST:
			request.getResponse().sendJsonMap(
					repository.createConfig(request, type));
			break;
		default:
			request.getResponse().sendJsonArray(
					repository.getConfigList(request, type));
		}
	}
	...
	protected void process(Request request, String type, String id) {
		...
		case PUT:
			...
			if(valid){
				request.getResponse().sendJsonMap(
						repository.updateConfig(request, type, id));
			}
			break;
		case DELETE:
			request.getResponse().sendJsonMap(
					repository.deleteConfig(request, type, id));
			break;
		default:
			request.getResponse().sendJsonMap(
					repository.readConfig(request, id));
		}
	}
